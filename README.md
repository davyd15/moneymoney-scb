...
> **Status: working, version 1.1.** Verified with savings account statements with and without transactions. See the changelog for what has been exercised.
...
| 8 | The counterparty name is taken from the description (`Transfer to KBNK x3984 Mrs. ...`, `... B/O David Lemke`), the description becomes the purpose, channel and code the booking text. MoneyMoney's auto-categorisation applies. |
...
1. Put at least one statement PDF into the inbox: the account list comes from the statements the bridge has read. For an account that has not moved lately, request a longer period (up to 12 months) so the statement lists at least one transaction and therefore a balance.
